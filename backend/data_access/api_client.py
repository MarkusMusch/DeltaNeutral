...
class ByBitClient:
    ...
    def _sign_request(self, params: dict) -> str:
        ...
        try:
            ...
            logger.debug("Signing request with params: %s", param_str)
    ...
    def get_interest_rate(self, currency: str, end_time: int) -> InterestRateResponse:
        ...
        try:
            logger.info("Fetching interest rate for currency: %s with params: %s", currency, params)
    ...
    def get_funding_history(self, params: FundingRequest) -> FundingHistoryResponse:
        ...
        try:
            logger.info("Fetching funding history with params: %s", params.model_dump())
    ...
    def get_open_interest(self, params: OpenInterestRequest) -> OpenInterestResponse:
        ...
        try:
            logger.info("Fetching open interest with params: %s", params.model_dump())
            response = requests.get(url, params=params.model_dump())
            response.raise_for_status()
            response_data = response.json()['result']
