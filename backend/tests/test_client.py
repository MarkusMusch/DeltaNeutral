...
from backend.models.models_api import FundingRequest, OpenInterestRequest, InterestRateResponse, FundingHistoryResponse, OpenInterestResponse
...
@pytest.fixture
def mock_client():
    with patch('backend.data_access.api_client.backend_settings') as mock_settings:
...
def mock_requests_get():
    with patch('backend.data_access.api_client.requests.get') as mock_get:
        yield mock_get


@pytest.fixture
def mock_time():
    with patch('backend.data_access.api_client.time.time') as mock_time_func:
...
@pytest.fixture
def mock_hmac():
    with patch('backend.data_access.api_client.hmac.new') as mock_hmac:
        yield mock_hmac


@pytest.fixture
def mock_hashlib():
    with patch('backend.data_access.api_client.hashlib.sha256') as mock_hashlib:
        yield mock_hashlib


@pytest.fixture
def mock_funding_history_response():
    with patch('backend.data_access.api_client.FundingHistoryResponse') as mock_response:
        yield mock_response


@pytest.fixture
def mock_open_interest_response():
    with patch('backend.data_access.api_client.OpenInterestResponse') as mock_response:
        yield mock_response


@pytest.fixture
def mock_interest_rate_response():
    with patch('backend.data_access.api_client.InterestRateResponse') as mock_response:
        yield mock_response
...
class TestByBitClient:
    ...
    def test_get_funding_rate_exceptions(self, mock_client):
        with patch('backend.data_access.api_client.RequestException') as mock_request_exception:
