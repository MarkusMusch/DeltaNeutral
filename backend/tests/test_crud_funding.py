...
def test_read_funding_entries_sqlalchemy_error():
...
def test_read_funding_entries_unexpected_error():
...
def test_read_most_recent_update_funding_sqlalchemy_error():
    symbol = Symbol('BTCUSDT') 
    ...
    with patch("backend.data_access.crud.crud_funding.Session") as mock_session:
        ...
        # Step 2: Call the function to test
        with pytest.raises(SQLAlchemyError) as exc_info:
            read_most_recent_update_funding(symbol)

        # Step 3: Verify that the error message is as expected
        assert "Database error" in str(exc_info.value)


def test_read_most_recent_update_funding_unexpected_error():
    symbol = Symbol('BTCUSDT')
